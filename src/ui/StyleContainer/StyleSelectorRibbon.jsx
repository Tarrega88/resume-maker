import StyleRowNav from "./StyleRowNav";

function StyleSelectorRibbon({ currentView, setView, views = [], children }) {
  const activeClass = "text-2xl";
  const inactiveClass =
    "cursor-pointer text-slate-400 transition-all duration-300 hover:scale-110 hover:text-slate-300";
  return (
    <div className="mx-6 h-max w-full">
      <StyleRowNav />
      ...
      <div className="flex h-full items-start justify-center rounded-b-md bg-slate-400 p-6 pt-24">
        {children}
      </div>
    </div>
  );
...
export default StyleSelectorRibbon;
